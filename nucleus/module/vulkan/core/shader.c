...
{
    shaderc_include_result *result = (shaderc_include_result*)nu_malloc(sizeof(shaderc_include_result));

    uint32_t nbytes;
    char *buf;
    if (nu_file_read_allbytes(requested_source, &nbytes, &buf) != NU_SUCCESS) {
...
)
{
    ...
    uint32_t nbytes;
    char *buf;
    if (nu_file_read_allbytes(filename, &nbytes, &buf) != NU_SUCCESS) {
        nu_error(NUVK_LOGGER_NAME"Failed to read file: %s.\n", filename);
        return NU_FAILURE;
    }

    /* compile to spirv */
    shaderc_compilation_result_t result = shaderc_compile_into_spv(manager->compiler, buf,
        nbytes, vulkan_stage_to_shaderc_kind(stage), filename, "main", manager->options);
    nu_free(buf);
    if (result == NULL) {
        ...
        nu_error("%s", shaderc_result_get_error_message(result));
        shaderc_result_release(result);
        return NU_FAILURE;
    }

    /* recover spirv code */
    *code_size = shaderc_result_get_length(result);
    *code      = (uint32_t*)nu_malloc(*code_size);
    memcpy(*code, shaderc_result_get_bytes(result), *code_size);
    shaderc_result_release(result);
...
)
{
    ...
    uint32_t code_size;
    uint32_t *code;
    if (glsl_to_spirv(manager, filename, stage, &code_size, &code) != NU_SUCCESS) {
