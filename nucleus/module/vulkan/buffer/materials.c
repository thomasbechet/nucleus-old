...
)
{
    buffer->uniform_buffer_range = nuvk_buffer_pad_uniform_buffer_size(context, 
        sizeof(nuvk_buffer_material_data_t) * NUVK_MAX_MATERIAL_COUNT);
...
{
    nuvk_buffer_material_data_t buf;
    nu_vec3f_copy(material->color, buf.color);

    nuvk_dynamic_range_buffer_write(
        &buffer->dynamic_range_buffer, active_inflight_frame_index,
        sizeof(nuvk_buffer_material_data_t) * material_index, sizeof(nuvk_buffer_material_data_t),
        &buf);
