        ...
        if (info.indice_count) {
            ...
            if (info.position_indices) {
                for (uint32_t i = 0; i < info.indice_count; i++) {
                    nu_vec3f_copy(info.positions[info.position_indices[i]], vertices[i].position);
                }
            }
            if (info.uv_indices) {
                for (uint32_t i = 0; i < info.indice_count; i++) {
                    nu_vec2f_copy(info.uvs[info.uv_indices[i]], vertices[i].uv);
                }
            }
            ...
            if (info.positions) {
                for (uint32_t i = 0; i < info.vertex_count; i++) {
                    nu_vec3f_copy(info.positions[i], vertices[i].position);
                }
            }
            if (info.uvs) {
                for (uint32_t i = 0; i < info.vertex_count; i++) {
                    nu_vec2f_copy(info.uvs[i], vertices[i].uv);
                }
            }
        }
