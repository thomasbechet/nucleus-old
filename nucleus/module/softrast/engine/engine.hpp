...
namespace nu::softrast
{
    ...
    public:
        ...
        nu_renderer_mesh_t createMesh(const nu_renderer_mesh_create_info_t &info);
        void destroyMesh(nu_renderer_mesh_t handle);

        nu_renderer_texture_t createTexture(const nu_renderer_texture_create_info_t &info);
        void destroyTexture(nu_renderer_texture_t handle);

        nu_renderer_material_t createMaterial(const nu_renderer_material_create_info_t &info);
        void destroyMaterial(nu_renderer_material_t handle);

        nu_renderer_font_t createFont(const nu_renderer_font_create_info_t &info);
        void destroyFont(nu_renderer_font_t handle);
        Vector2u getFontTextSize(nu_renderer_font_t handle, const std::string &text);

        nu_renderer_model_t createModel(const nu_renderer_model_create_info_t &info);
        void destroyModel(nu_renderer_model_t handle);
        void setModelTransform(nu_renderer_model_t handle, const Matrix4f &transform);
