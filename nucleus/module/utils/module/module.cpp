...
nu_result_t nu_plugin_get_interface(nu_plugin_interface_t *interface, const char *name)
{
    if (NU_MATCH(name, NUUTILS_CONSOLE_PLUGIN_NAME)) {
        ...
        interface->initialize = nuutils_spectator_plugin_initialize;
        interface->terminate  = nuutils_spectator_plugin_terminate;
        interface->update     = nuutils_spectator_plugin_update;
        return NU_SUCCESS;
    }
