...
void ColorFramebuffer::setColor(uint32_t x, uint32_t y, const nu_vec4f_t color)
{
    nu_vec4f_copy(color, m_data[y * m_width + x].data);
}

void ColorFramebuffer::clear(const nu_vec4f_t color)
{
    Color4 clearColor;
    nu_vec4f_copy(color, clearColor.data);
