...
nu_time_t nu_time_now(void)
{
    return time(NULL);
}
void nu_time_format(nu_time_t time, char buf[NU_TIME_FORMAT_LENGTH])
{
    time_t t = time;
    struct tm ts = *localtime(&t);
    strftime(buf, sizeof(char) * NU_TIME_FORMAT_LENGTH, "%Y-%m-%d %H:%M:%S", &ts);
}
