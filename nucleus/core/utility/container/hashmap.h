#ifndef NU_HASHMAP_H
#define NU_HASHMAP_H

#include <nucleus/core/utility/platform.h>
#include <nucleus/core/utility/macro.h>

// NU_DECLARE_HANDLE(nu_hashmap_t);
// typedef nu_result_t (*nu_hashmap_hash_pfn_t)(const void*, const void*);
// typedef nu_result_t (*nu_hashmap_equals_pfn_t)(const void*);

// NU_API void nu_hashmap_allocate(nu_hashmap_t *hashmap, uint32_t key_size, uint32_t value_size, nu_hashmap_hash_pfn_t hash);
// NU_API void nu_hashmap_free(nu_hashmap_t hashmap);
// NU_API void nu_hashmap_put(nu_hashmap_t hashmap, const void *key, const void *value);
// NU_API void *nu_hashmap_get(nu_hashmap_t hashmap, const void *key);
// NU_API void nu_hashmap_remove(nu_hashmap_t hashmap, const void *key);

#endif