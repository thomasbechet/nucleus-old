...
nu_result_t nu_json_allocate_empty_object(nu_json_t *json)
{
    *json = (nu_json_t)cJSON_CreateObject();
    return NU_SUCCESS;
}
nu_result_t nu_json_allocate_empty_array(nu_json_t *json)
{
    *json = (nu_json_t)cJSON_CreateArray();
    return NU_SUCCESS;
}
...
nu_result_t nu_json_value_as_quatf(nu_json_value_t value, nu_quatf_t q)
{
    ...
    while (nu_json_array_next(array, &it) && index <= 3) {
        ...
        NU_CHECK(result == NU_SUCCESS, return NU_FAILURE, NU_LOGGER_NAME, "Json quatf %d component is not a float.", index);
        index++;
    }
...
nu_result_t nu_json_value_as_transform(nu_json_value_t value, nu_transform_t *transform)
{
    ...
    if (j_rotation != NU_NULL_HANDLE) {
        ...
        NU_CHECK(result == NU_SUCCESS, return NU_FAILURE, NU_LOGGER_NAME, "Failed to parse rotation component for transform.");
    }
