#ifndef NU_API_INPUT_H
#define NU_API_INPUT_H
...
typedef struct {
    nu_button_state_t (*get_keyboard_state)(nu_keyboard_t keyboard);
    void (*get_keyboard_text)(const char **text, uint32_t *length);
    nu_button_state_t (*get_mouse_state)(nu_mouse_t mouse);
    void (*get_mouse_motion)(nu_vec2f_t motion);
    void (*get_mouse_scroll)(nu_vec2f_t scroll);
    nu_cursor_mode_t (*get_cursor_mode)(void);
    void (*set_cursor_mode)(nu_cursor_mode_t mode);
} nu_input_api_t;
...
} nu_input_system_api_t;

typedef enum {
    NU_INPUT_MODULE_NONE = 0,
    NU_INPUT_MODULE_GLFW = 1
} nu_input_module_t;
