...
{
    nu_log(severity, NU_LOGGER_NAME, "[DEBUG MEMORY] [alloc:%llu free:%llu realloc:%llu]",
        _system.alloc_count, _system.free_count, _system.realloc_count);
    nu_log(severity, NU_LOGGER_NAME, "[DEBUG MEMORY] [alloc-aligned:%llu free-aligned:%llu realloc-aligned:%llu]",
...
nu_result_t nu_memory_terminate(void)
{
    ...
    if (_system.alloc_count != _system.free_count) {
        ...
        nu_warning(NU_LOGGER_NAME, "[DEBUG MEMORY] Memory leak detected.");
    } else {
        nu_memory_log_status(NU_INFO);
        nu_info(NU_LOGGER_NAME, "[DEBUG MEMORY] No memory leak detected.");
    }
#endif
...
void nu_free(void *p)
{
    ...
    if (_system.free_count >= _system.alloc_count) {
        ...
        nu_interrupt(NU_LOGGER_NAME, "[DEBUG MEMORY] Free on non allocated memory detected. Exiting...");
    }
    _system.free_count++;
#endif
    free(p);
...
void nu_free_aligned(void *p)
{
    ...
    if (_system.free_aligned_count >= _system.alloc_aligned_count) {
        nu_memory_log_status(NU_FATAL);
        nu_interrupt(NU_LOGGER_NAME, "[DEBUG MEMORY] Free on non allocated aligned memory detected. Exiting...");
    }
