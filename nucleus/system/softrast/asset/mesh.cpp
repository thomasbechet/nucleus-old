    ...
    for (uint32_t meshIndex = 0; meshIndex < info.submesh_count; meshIndex++) {
        ...
        if (meshInfo.indice_count > 0) {
            for (uint32_t i = 0; i < meshInfo.indice_count; i++) {
                ...
                nu_vec3_copy(meshInfo.positions[position_indice], vertex.position);
                nu_vec2_copy(meshInfo.uvs[uv_indice], vertex.uv);
                vertices.emplace_back(vertex);
            }
        } else {
            for (uint32_t i = 0; i < meshInfo.vertex_count; i++) {
                Vertex vertex;
                nu_vec3_copy(meshInfo.positions[i], vertex.position);
                nu_vec2_copy(meshInfo.uvs[i], vertex.uv);
                vertices.emplace_back(vertex);
            }
        }
