...
{
    // Initialize camera
    const nu_vec3_t center = {0.0f, 0.0f, -1.0f};
    const nu_vec3_t up     = {0.0f, 1.0f, 0.0f};
    nu_vec3_zero(m_camera.eye);
    nu_vec3_copy(center, m_camera.center);
    nu_vec3_copy(up, m_camera.up);
...
void Scene::setCameraEye(const nu_vec3_t eye)
{
    nu_vec3_copy(eye, m_camera.eye);
}
void Scene::setCameraCenter(const nu_vec3_t center)
{
    nu_vec3_copy(center, m_camera.center);
}
...
void Scene::setModelTransform(nu_renderer_model_handle_t handle, const nu_mat4_t transform)
{
    nu_mat4_copy(transform, m_models.at(NU_HANDLE_GET_ID(handle)).transform);
}
...
{
    // Multiply MVP
    nu_vec4(input.position, 1.0f, position);
    nu_mat4_mulv(transform, position, position);
    
    // Copy uv
    nu_vec2_copy(input.uv, uv);
}
...
static void positionToViewport(const nu_vec4_t vp, nu_vec2_t v)
{
    /* (p + 1) / 2 */
...
static float pixelCoverage(const nu_vec2_t a, const nu_vec2_t b, const nu_vec2_t c)
{
...
void Scene::draw(ColorFramebuffer &colorBuffer, Framebuffer<float> &depthBuffer, const AssetManager &assetManager) const
{
    ...
    const nu_vec4_t viewport = {0, 0, (float)width, (float)height};

    // Compute VP matrix
    nu_mat4_t camera_projection, camera_view, vp;
    const nu_vec3_t up = {0.0f, 1.0f, 0.0f};
    nu_lookat(m_camera.eye, m_camera.center, up, camera_view);
    ...
    for (auto &any : m_models) {
        ...
        // Compute mvp matrix
        nu_mat4_t mvp;
        nu_mat4_mul(vp, model.transform, mvp);
                ...
                for (uint32_t idx = 0; idx < indiceCount; idx += 3) {
                    ...
                    for (uint32_t j = tvp[1]; j < tvp[3]; j++) {
                        for (uint32_t i = tvp[0]; i < tvp[2]; i++) {
                            nu_vec2_t sample = {i + 0.5f, j + 0.5f};
