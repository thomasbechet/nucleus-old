...
nu_result_t nusr_mesh_create(uint32_t *id, const nusr_mesh_create_info_t *info)
{
    ...
    if (info->indices) {
        for (uint32_t i = 0; i < info->vertice_count; i++) {
            uint32_t indice = info->indices[i];
            glm_vec3_copy(info->positions[indice], _data.meshes[_data.next_id]->positions[i]);
            glm_vec2_copy(info->uvs[indice], _data.meshes[_data.next_id]->uvs[i]);
            glm_vec3_copy(info->colors[indice], _data.meshes[_data.next_id]->colors[i]);
        }
    } else {
